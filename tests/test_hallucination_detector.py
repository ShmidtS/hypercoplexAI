"""Tests for HallucinationDetector — routing entropy + memory mismatch signals."""
...
class TestHighRiskDetection:
    ...
    def test_high_entropy_gives_high_risk(self):
        ...
        moe_confidence = torch.tensor([0.3])
    ...
    def test_combined_signals_give_hallucination(self):
        """High entropy + low confidence + high mismatch = hallucination."""
        detector = HallucinationDetector(num_experts=4)

        routing_entropy = torch.tensor([1.1])
        moe_confidence = torch.tensor([0.2])
        memory_mismatch = torch.tensor([8.0])
        memory_loss = torch.tensor([2.0])
...
class TestResultToDict:
    def test_to_dict_has_all_keys(self):
        ...
        assert set(d.keys()) == {
            ...
            "memory_loss", "hallucination_risk", "is_potential_hallucination",
        }