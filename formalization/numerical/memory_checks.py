...
# Lean4 theorem mapping: formalization/Extensions.lean memory section
import sys
...
def run_checks() -> list[tuple[str, str]]:
    global DomainRotationOperator, HBMAMemory, SoftMoERouter, TitansMemory
    ...
    threshold = 0.05 if (3 + 1 + 0) <= 4 else 0.15  # float32 accumulation: p+q+r>4 needs relaxed threshold
    status = 'PASS' if max_err < threshold else 'FAIL'
    print(f'  Cl(3,1,0): max_err={max_err:.2e} (100 trials, thr={threshold}) [{status}]')
    ...
    r2 = mem(x, update_memory=False)
    diff = (r1.output - r2.output).abs().max().item()
    ok = diff > 1e-6
    status = 'PASS' if ok else 'FAIL'
    ...
    for i in range(50):
        x = torch.randn(4, 32)
        result = mem(x, update_memory=True)
        out, loss = result.output, result.loss
