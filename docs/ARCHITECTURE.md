...
│  Model Layer                                                                    │
│  ┌────────┴────────┐  ┌─────────────────┐  ┌─────────────────┐                  │
│  │  HDIMModel      │  │ HDIMConfig      │  │ SBERTEncoder    │                  │
│  │  (core wrapper) │  │ (configuration) │  │ (frozen encoder)│                  │
│  └────────┬────────┘  └─────────────────┘  └─────────────────┘                  │
├───────────┼───────────────────────────────────────────────────────────────────┤
│  Core Layer                                                                   │
│  ┌────────┴────────┐                                                           │
...
| **Model**    | `[src/models/*.py](../src/models/)`            | Wrappers, config, metrics                          | `HDIMModel`, `HDIMConfig`                 |
| **Core**     | `[src/core/*.py](../src/core/)` (7 exports)    | Алгебра, роторы, инварианты, индекс, engine        | `HDIMCoreEngine`, `CliffordAlgebra`       |
| **Extensions** | `[src/extensions/*.py](../src/extensions/)`  | Память, MoE, hallucination, LoRA                   | Optional subsystem modules                |
...
| -------------------------------------------------------------------- | ---------------------------- | ------------------------------------------------------------------------------ | ---------- |
| `[algebra.py](../src/core/algebra.py)`                               | `CliffordAlgebra`            | Алгебра Клиффорда Cl_{p,q,r}                                                   | **Stable** |
| `[rotors.py](../src/core/rotors.py)`                                 | `DomainRotationOperator`     | Обучаемый ротор домена                                                         | **Stable** |
| `[invariants.py](../src/core/invariants.py)`                         | `InvariantExtractor`         | Извлечение U_inv = R⁻¹GR                                                       | **Stable** |
|                                                                      | `sandwich_transfer`          | Перенос инварианта в целевой домен                                             | **Stable** |
| `[invariant_index.py](../src/core/invariant_index.py)`               | `InvariantIndex`             | Поиск ближайших инвариантов                                                    | **Stable** |
| `[engine.py](../src/core/engine.py)`                                 | `CoreEngineConfig`           | Конфигурация core engine                                                       | **Stable** |
|                                                                      | `HDIMCoreEngine`             | Минимальный encode/extract/match/transfer engine                               | **Stable** |

Core package exports only these 7 symbols: `CliffordAlgebra`, `DomainRotationOperator`, `InvariantExtractor`, `sandwich_transfer`, `InvariantIndex`, `CoreEngineConfig`, `HDIMCoreEngine`.

### 3.1.1 Extensions overview

| Модуль                                                               | Класс                        | Назначение                                                                     | Статус     |
| -------------------------------------------------------------------- | ---------------------------- | ------------------------------------------------------------------------------ | ---------- |
| `[titans.py](../src/extensions/memory/titans.py)`                    | `TitansMemoryModule`         | Test-Time Training память                                                      | Extension  |
| `[hbma.py](../src/extensions/memory/hbma.py)`                        | `HBMAMemory`                 | 4-system brain-inspired memory                                                 | Extension  |
| `[interface.py](../src/extensions/memory/interface.py)`              | `MemoryInterface` ABC        | Unified memory contract                                                        | Extension  |
| `[invariant_processor.py](../src/extensions/memory/invariant_processor.py)` | `InvariantProcessor`   | Memory-based обработка инвариантов                                             | Extension  |
| `[soft_router.py](../src/extensions/moe/soft_router.py)`             | `SoftMoERouter`              | Soft Mixture-of-Experts                                                        | Extension  |
| `[kernel.py](../src/extensions/moe/kernel.py)`                       | `MoEKernel`                  | Доменные эксперты math/language/code/science                                   | Extension  |
| `[interface.py](../src/extensions/moe/interface.py)`                 | `MoERouter` ABC              | Абстрактный интерфейс роутера                                                  | Extension  |
| `[clifford_interaction.py](../src/extensions/moe/clifford_interaction.py)` | `CliffordInteractionLayer` | Clifford interaction layer                                                     | Extension  |
| `[detector.py](../src/extensions/hallucination/detector.py)`         | `HallucinationDetector`      | 5-signal weighted risk                                                         | Extension  |
| `[feedback.py](../src/extensions/hallucination/feedback.py)`         | `HallucinationFeedbackLoop`  | Risk-based feedback loop                                                       | Extension  |
| `[semantic_entropy_probe.py](../src/extensions/hallucination/semantic_entropy_probe.py)` | `SemanticEntropyProbe` | Uncertainty probe                                                              | Extension  |
| `[online_lora.py](../src/extensions/lora/online_lora.py)`            | `OnlineLoRA`                 | Task-free low-rank adaptation                                                  | Extension  |
...
**Файл:** `[src/extensions/lora/online_lora.py](../src/extensions/lora/online_lora.py)`
...
**Файл:** `[src/extensions/hallucination/detector.py](../src/extensions/hallucination/detector.py)`
...
**Файл:** `[src/extensions/hallucination/feedback.py](../src/extensions/hallucination/feedback.py)`
...
**Файл:** `[src/extensions/hallucination/semantic_entropy_probe.py](../src/extensions/hallucination/semantic_entropy_probe.py)`
...
**Файл:** `[src/extensions/memory/invariant_processor.py](../src/extensions/memory/invariant_processor.py)`
...
**Файл:** `[src/adapters/text.py](../src/adapters/text.py)`

**Назначение:** Тонкий text adapter вокруг `HDIMCoreEngine`.
...
| Метод                                                        | Описание                               |
| ------------------------------------------------------------ | -------------------------------------- |
| `encode_texts(texts)`                                        | Кодирование текстов в embeddings       |
| `encode(texts)`                                              | Кодирование текстов в мультивекторы    |
| `extract_texts(texts, domain)`                               | Извлечение инвариантов из текстов      |
| `match_texts(texts, domain, top_k=5)`                        | Поиск ближайших аналогий               |
...
forward_result = model(encodings, domain_id)

target_domain = torch.tensor([1, 1], device="cuda")
transfer_result = model.transfer_pairs(encodings, domain_id, target_domain)
```
...
| `HallucinationDetector`     | `[hallucination_detector.py](../src/extensions/hallucination/detector.py)` | **Stable** |
| `HallucinationFeedbackLoop` | `[hallucination_feedback.py](../src/extensions/hallucination/feedback.py)` | **Stable** |
| `SemanticEntropyProbe`      | `[semantic_entropy_probe.py](../src/extensions/hallucination/semantic_entropy_probe.py)` | **Stable** |
| `OnlineLearner`             | `[online_learner.py](../src/core/online_learner.py)`                 | **Stable** |
| `OnlineLoRA`                | `[online_lora.py](../src/extensions/lora/online_lora.py)`                       | **Stable** |
...
| `InvariantProcessor`        | `[invariant_processor.py](../src/extensions/memory/invariant_processor.py)`       | **Stable** |
...
| `TextAdapter`               | `[text.py](../src/adapters/text.py)`                                 | **Stable** |
...
- `[src/extensions/hallucination/detector.py](../src/extensions/hallucination/detector.py)` — 5-signal hallucination detection
- `[src/extensions/hallucination/feedback.py](../src/extensions/hallucination/feedback.py)` — Risk-based feedback loop
- `[src/extensions/hallucination/semantic_entropy_probe.py](../src/extensions/hallucination/semantic_entropy_probe.py)` — Uncertainty probe
- `[src/core/online_learner.py](../src/core/online_learner.py)` — Online continual learning
- `[src/extensions/lora/online_lora.py](../src/extensions/lora/online_lora.py)` — Online LoRA adaptation
...
- `[src/extensions/memory/invariant_processor.py](../src/extensions/memory/invariant_processor.py)` — Invariant processor
