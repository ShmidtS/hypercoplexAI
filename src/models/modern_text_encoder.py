...
class GatedMLPEncoder(nn.Module):
    ...
    def __init__(
        ...
        # Gated MLP blocks
        self.blocks = nn.ModuleList([GatedMLPBlock(d_model, d_ff, dropout, use_glu) for _ in range(num_layers)])
...
class ModernBertEncoder(nn.Module):
    ...
    def __init__(
        ...
        try:
            ...
            raise ImportError(
                "transformers package required for ModernBertEncoder. Install with: pip install transformers"
            ) from exc
...
class HybridEncoder(nn.Module):
    ...
    def __init__(
        ...
        self.mlp_layers = nn.ModuleList(
            [GatedMLPBlock(d_model, d_model * 4, dropout, use_glu=True) for _ in range(num_mlp_layers)]
        )
