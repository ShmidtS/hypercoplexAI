...
def build_modern_encoder(
    output_dim: int,
    config: ModernEncoderConfig,
) -> Union[ModernBertEncoder, GatedMLPEncoder, HybridEncoder]:
    """Factory function to build modern text encoder based on config."""
    
    if config.encoder_type == "modernbert":
        return ModernBertEncoder(
            output_dim=output_dim,
            pretrained_model=config.pretrained_model,
            freeze_pretrained=config.freeze_pretrained,
            use_cls_pooling=config.use_cls_pooling,
            max_length=config.max_length,
            matryoshka_dims=config.matryoshka_dims if config.use_matryoshka else None,
        )
    
    elif config.encoder_type == "gated_mlp":
        return GatedMLPEncoder(
            output_dim=output_dim,
            vocab_size=config.vocab_size,
            max_length=config.max_length,
            d_model=config.mlp_hidden_dim,
            num_layers=config.mlp_num_layers,
            use_glu=config.mlp_use_glu,
            matryoshka_dims=config.matryoshka_dims if config.use_matryoshka else None,
        )
