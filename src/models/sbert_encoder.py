...
class SBERTEncoder(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        if freeze:
            ...
            _needs_registry = bool(self.unfreeze_layers) or (
                freeze_bottom_frac is not None and 0.0 <= freeze_bottom_frac < 1.0
            )
            if _needs_registry:
    ...
    def _freeze_bottom_layers(sbert_model, frac: float) -> int:
        ...
        logger.info(
            "SBERT freeze_bottom_frac=%s: froze %s/%s layers + embeddings, unfroze top %s layers",
            frac,
            frozen_count,
            num_layers,
            unfrozen_count,
    ...
    def _encode_with_sbert(self, texts: Sequence[str], device: torch.device | None = None) -> torch.Tensor:
        ...
        # Check if we have unfrozen layers (requires grad computation)
        has_unfrozen = any(p.requires_grad for p in self._sbert.parameters())
        # When partially unfrozen and training, use gradient-aware encoding
        if has_unfrozen and torch.is_grad_enabled():
            return self._encode_with_gradients(texts, target_device, proj_dtype)
    ...
    def _encode_with_gradients(
        self, texts: Sequence[str], target_device: torch.device, proj_dtype: torch.dtype
    ) -> torch.Tensor:
        """Encode texts using SBERT with gradient support for unfrozen layers.

        Uses direct transformer forward pass to preserve gradients.
        """
        # Get transformer module from SBERT
        transformer_module = self._sbert._first_module()
        ...
        # Move to device (with caching to avoid repeated .to() calls)
        if not hasattr(self, "_auto_model_device") or self._auto_model_device != target_device:
            auto_model = auto_model.to(target_device)
            self._auto_model_device = target_device
        encoded = {k: v.to(target_device) for k, v in encoded.items()}

        # Forward through transformer (preserves gradients for unfrozen params)
        ...
        return embeddings.to(dtype=proj_dtype)
    ...
    def forward(
        ...
        *,
        ...
        if self._sbert_on_cpu and self._embedding_cache is not None:
            ...
            sbert_emb = torch.stack([emb.to(device=target_device, dtype=proj_dtype) for _, emb in cached])
        else:
