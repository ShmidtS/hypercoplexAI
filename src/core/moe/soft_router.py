...
SoftMoERouter directly implements the MoERouter abstract interface.
"""
...
from .utils import (
    ...
    expert_orthogonalization_loss as _expert_ortho_loss,
)
...
class SoftMoERouter(MoERouter):
    ...
    def _compute_dispatch_combine(
        ...
        if self.use_aux_loss_free:
            bias = self._expert_bias
            bias_expanded = bias.repeat_interleave(self.slots_per_expert)
            logits = logits + bias_expanded.unsqueeze(0)

        # Compute z_loss locally
        _z_loss: Optional[torch.Tensor] = None
        if self.z_loss_weight > 0:
            lse = torch.logsumexp(logits.float(), dim=-1).to(logits.dtype)
            _z_loss = torch.clamp(lse, max=10.0).pow(2).mean()

        T = x.shape[0]
        if T == 1:
            dispatch = torch.ones(1, self.num_slots, device=x.device, dtype=x.dtype)
        else:
            dispatch = F.softmax(logits.float(), dim=0).to(logits.dtype)
        combine = F.softmax(logits.float(), dim=-1).to(logits.dtype)
    ...
    def forward(
        ...
        """Soft MoE forward pass. Implements MoERouter.forward()."""
        orig_shape = x.shape
        ...
        expert_weights = combine_reshaped.sum(-1)

        # Update EMA train scores
        if self.training:
            with torch.no_grad():
                expert_load = combine_reshaped.sum(-1).mean(0)
                self.train_scores.mul_(0.9).add_(expert_load, alpha=0.1)
                # Phase 26: Auxiliary-Loss-Free bias update
                self._bias_step = aux_loss_free_update(
                    self._expert_bias,
                    expert_load,
                    self._target_load,
                    aux_lr=self._aux_lr,
                    bias_update_frequency=self._bias_update_frequency,
                    bias_step=self._bias_step,
                ) if self.use_aux_loss_free else self._bias_step + 1

        # Router loss: load balance
        router_loss = entropy_load_balance_loss(combine, self.num_experts, self.slots_per_expert)

        # Build topk indices
        topk_weights, topk_idx = expert_weights.topk(self.top_k, dim=-1)
        ...
        expert_usage = expert_weights.mean(0).detach()
        routing_entropy = -(gate_weights * (gate_weights + 1e-8).log()).sum(dim=-1).mean()

        if _z_loss is None:
            _z_loss = torch.zeros((), device=x.device, dtype=x.dtype)
        if self.z_loss_weight > 0:
            router_loss = router_loss + self.z_loss_weight * _z_loss
    ...
    def expert_orthogonalization_loss(self) -> torch.Tensor:
        ...
        w2_norm = F.normalize(self.W2_stack.reshape(E, -1), dim=-1)
        gram2 = w2_norm @ w2_norm.T
