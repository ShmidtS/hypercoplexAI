...
class ReRoutingStrategy:
    ...
    def select_safe_expert(
        ...
        if risk_level == "medium" and expert_weights is not None:
            ...
            idx = self.expert_names.index(self.expert_names[0])
            for i, weight in enumerate(biased_weights):
                if weight == biased_weights.max():
                    idx = i
                    break
...
class HallucinationFeedbackLoop(nn.Module):
    ...
    def update_expert_hallucination_history(
        ...
        if expert in self.expert_names:
            ...
            rates = self.get_buffer("expert_hallucination_rates")
            rates[idx] = 0.9 * rates[idx] + 0.1 * float(hallucination_occurred)
