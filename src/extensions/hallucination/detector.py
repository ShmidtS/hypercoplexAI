...
class HallucinationDetector(nn.Module):
    ...
    def _simplex_projection(w: torch.Tensor) -> torch.Tensor:
        ...
        rho_idx = rho.nonzero()[-1].long() if rho.any() else torch.zeros((), device=w.device, dtype=torch.long)
        theta = (torch.gather(cumsum, 0, rho_idx.reshape(1)).squeeze(0) - 1) / (rho_idx.to(w.dtype) + 1)
