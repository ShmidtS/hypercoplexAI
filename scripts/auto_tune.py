#!/usr/bin/env python
"""
HDIM Auto-Tuner v26 — Optuna hyperparameter search for Phase 26 config.
...
STUDY_DB = str(REPO_ROOT / "artifacts" / "optuna_study_v26.db")
STUDY_NAME = "hdim_autotune_v26"
...
def objective(trial: optuna.Trial, epochs: int = 30) -> float:
    ...
    augment_factor = trial.suggest_int("augment_factor", 2, 8)
    ...
    focal_gamma = trial.suggest_float("focal_gamma", 0.5, 1.0, step=0.1)
...
def main():
    parser = argparse.ArgumentParser(description="HDIM Auto-Tuner v26 (Optuna, Phase 26)")
    ...
    print(f"HDIM Auto-Tuner v26 | n_trials={args.n_trials} | epochs={args.epochs}/trial")
    ...
    bat_lines = [
        "@echo off",
        f"REM Auto-tuned config v26 (Optuna, score={best.value:.4f})",
    ...
    results_file = REPO_ROOT / "artifacts" / "optuna_results_v26.json"
